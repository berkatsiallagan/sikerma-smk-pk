...
<body class="flex text-black font-sans">
  ...
  <div class="flex flex-col flex-grow p-8 items-start text-left bg-gray-100">
    <h3 class="text-4xl font-semibold text-gray-800 mb-6">Tambah Bidang</h3>

    <main class="bg-white p-6 rounded-3xl w-full">
      <form action="/kelola-bidang" method="POST">
        @csrf
        <div class="mb-2">
          <label for="id_bidang" class="block text-sm font-medium text-gray-700 mb-1 required-field">No.</label>
          <input type="text" required id="id_bidang" name="id_bidang" placeholder="Masukkan no. bidang" value="{{ old('id_bidang') }}"
        ...
        <div class="mb-2">
          ...
          <input type="text" required id="nama_bidang" name="nama_bidang" placeholder="Masukkan nama bidang" value="{{ old('nama_bidang') }}"
                 class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
        </div>
